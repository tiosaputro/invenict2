            ...
            <div class="card-body">
                <form @submit.prevent="UpdateIct">
                    <div class="field grid">
                        ...
                        <div class="col">
                            <InputText type="text" v-model="request.ireq_no" disabled />
                            <InputText type="hidden" v-model="request.ireq_no" disabled />
                        </div>
                    </div>
                    <div class="field grid">
                        <label class="col-fixed w-9rem" style="width:120px">Priority Level</label>
                        <div class="col-fixed w-9rem">
                            <Dropdown v-model="request.ireq_prio_level" :options="level" optionLabel="name"
                                optionValue="code" placeholder="Select One" :showClear="true" :filter="true"
                                :class="{ 'p-invalid': error.ireq_prio_level }" autofocus />
                    ...
                    <div class="field grid">
                        ...
                        <div class="col-fixed w-9rem">
                            <InputText type="text" v-model="request.ireq_requestor" disabled />
                        </div>
                    </div>
                    <div class="field grid">
                        <label class="col-fixed w-9rem" style="width:120px">User</label>
                        <div class="col-fixed w-9rem">
                            <Dropdown v-model="request.ireq_user" :options="userList" optionLabel="usr_fullname"
                                optionValue="usr_domain" @change="getDataBuDvision(request.ireq_user)"
                                :class="{ 'p-invalid': error.ireq_user }" placeholder="Select One" :filter="true"
                                :showClear="true" />
                ...
                <label class="col-fixed w-9rem">Your Supervisor</label>
                  <div class="col-fixed w-9rem">
                    ...
                    <div class="form-group">
                        <Button v-if="this.loading == false" class="p-button-rounded p-button-primary mr-2"
                            icon="pi pi-check" label="Save" type="submit" />
                        <Button v-if="this.loading == false" label="Cancel"
                            class="p-button-rounded p-button-secondary mt-2" icon="pi pi-times"
    ...
    export default {
        ...
        methods: {
            ...
            getDataBuDvision(code) {
              this.request.division_user = this.getDataById(code, 'usr_division');
              this.request.bu_user = this.getDataById(code, 'usr_bu');
              this.request.department_user = this.getDataById(code, 'usr_department');
            },
            cekUser() {
            ...
            UpdateIct() {
                ...
                if (
                    ...
                    this.axios.post('/api/save-request', this.request).then(() => {
                        this.$toast.add({
                            severity: "success",
