<template>
    ...
    <div v-else>
        <DataTable :value="value" :paginator="true" :rows="10" :loading="loading" :filters="filters" :rowHover="true"
            ...
            <Column>
                <template #body="slotProps">
                    <Button v-if="slotProps.data.ireq_value == null && slotProps.data.status == 'C'" v-tooltip.bottom="'Click to give feedback'" class="p-button-raised p-button p-button-sm mr-2" label = "Give Feedback" @click="createFeedback(slotProps.data.ireqd_id,slotProps.data.ireq_id)" />
                    <Button label="Pdf" class="p-button-raised p-button-danger p-button-sm mt-2" v-tooltip.bottom="'Click to print out (PDF)'" icon="pi pi-file-pdf" @click="CetakPdf(slotProps.data.ireq_id)" />
                </template>
            </Column>
            <template #footer>
