...
class MutasiController extends Controller
{
    ...
    public function save(Request $request){
        $saveMutasi = Mutasi::create([
            'imutasi_id'=> generate_id(),
            'invent_code_dtl'=> $request->invent_sn,
            'imutasi_tgl_dari' => Carbon::parse($request->fromdate),
            'imutasi_tgl_sd' => $request->todate ? Carbon::parse($request->fromdate) : null,
    ...
    public function edit($code){
        $data['mut'] = $this->mutasiServices->getDataWithFilter($code);
        $data['listUser'] = Mng_user::orderBy('usr_fullname','ASC')->get();
        return ResponseFormatter::success($data,'Successfully get data');
    }
    public function update(Request $request, $code){
        ...
        $mut->imutasi_pengguna = $request->user_id;
