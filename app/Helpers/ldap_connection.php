...
class ldap_connection{
    public $ldapHost, $port, $baseDn, $upn, $config;
    ...
    public function getAllData($filter, $attributes = array()){
        ...
        if ($ldapconn) {
            ...
            if ($ldapbind) {
                    $attributes = [
                        'displayname', 'description', 'givenname', 'telephonenumber', 'whencreated', 'whenchanged', 'department', 'company', 'streetaddress',
                        'extensionattribute12', 'extensionattribute15', 'name', 'objectguid', 'employeeid', 'samaccountname', 'userprincipalname',
                        'mail', 'msexchwhenmailboxcreated', 'physicaldeliveryofficename', 'division', 'mobile', 'useraccountcontrol'
                    ];
                $results = ldap_search($ldapconn, $baseDn, $filter, $attributes);
                $info = ldap_get_entries($ldapconn, $results);
                $entry_array = $info;
                ...
                return (empty($person_array) ?  null : $person_array);
                ldap_unbind($ldapconn);
            }
        }
        ldap_close($ldapconn);
